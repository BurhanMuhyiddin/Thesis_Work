...
class main():
    def __init__(self):
        ...
        with sm:
            ...
            smach.StateMachine.add('PROCESS_IMAGE', smach_ros.ServiceState('/sample_rope', SampleRope,
                                    ...
                                    transitions={'pick_obj' : 'GO_HOLD_PICK_POSITION', 'place_obj' : 'PLACE_POSITION', 'aborted' : 'HOME'})
            ...
            smach.StateMachine.add('GO_HOLD_PICK_POSITION', smach_ros.ServiceState("/go_to_goal", GoToGoal,
                                    ...
                                    transitions={'succeeded' : 'HOLD_POSITION', 'preempted' : '', 'aborted' : 'HOME'})
            ...
            smach.StateMachine.add('HOLD_POSITION', smach_ros.ServiceState("/go_to_goal", GoToGoal,
                                    ...
                                    transitions={'succeeded' : 'PICK_POSITION', 'preempted' : '', 'aborted' : 'HOME'})
            ...
            smach.StateMachine.add('PICK_POSITION', smach_ros.ServiceState("/go_to_goal", GoToGoal,
                                    ...
                                    transitions={'succeeded' : 'PLACE_POSITION', 'preempted' : '', 'aborted' : 'HOME'})
            ...
            smach.StateMachine.add('PLACE_POSITION', smach_ros.ServiceState("/go_to_goal", GoToGoal,
                                    ...
                                    transitions={'home' : 'HOME', 'home_inter' : 'HOME_INTER', 'preempted' : '', 'aborted' : 'HOME'})
    ...
    def process_image_result_cb(self, ud, result):
        ...
        if -(m.pi/2) <= self.hold_angle <= 0:
            self.hold_angle = self.hold_angle
        elif -m.pi <= self.hold_angle <= -(m.pi/2):
            self.hold_angle = self.hold_angle
        elif m.pi/2 <= self.hold_angle <= m.pi:
            self.hold_angle = self.hold_angle
        elif 0 <= self.hold_angle <= m.pi/2:
            self.hold_angle = self.hold_angle
        ...
        # if -(m.pi/2) <= self.pick_angle <= 0:
        #     self.pick_angle = m.pi - abs(self.pick_angle)
        # elif -m.pi <= self.pick_angle <= -(m.pi/2):
        #     self.pick_angle = m.pi - abs(self.pick_angle)
        # elif m.pi/2 <= self.pick_angle <= m.pi:
        #     self.pick_angle = self.pick_angle - m.pi
        #     self.pick_angle = m.pi - abs(self.pick_angle)
        # elif 0 <= self.pick_angle <= m.pi/2:
        #     self.pick_angle = self.pick_angle - m.pi
        #     self.pick_angle = m.pi - abs(self.pick_angle)
    ...
    def place_pos_result_clb(self, ud, result):
        if result.success:
            finished_flag = False
            # rospy.loginfo("Total points: " + str(self.total_points) + "indexxxx: " + str(self.index))
