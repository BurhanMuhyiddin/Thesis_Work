// Auto-generated. Do not edit!

// (in-package baxter_msgs_mine.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;
let geometry_msgs = _finder('geometry_msgs');

//-----------------------------------------------------------

class GoToPointGoal {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.desired_pose = null;
      this.limb = null;
    }
    else {
      if (initObj.hasOwnProperty('desired_pose')) {
        this.desired_pose = initObj.desired_pose
      }
      else {
        this.desired_pose = [];
      }
      if (initObj.hasOwnProperty('limb')) {
        this.limb = initObj.limb
      }
      else {
        this.limb = '';
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type GoToPointGoal
    // Serialize message field [desired_pose]
    // Serialize the length for message field [desired_pose]
    bufferOffset = _serializer.uint32(obj.desired_pose.length, buffer, bufferOffset);
    obj.desired_pose.forEach((val) => {
      bufferOffset = geometry_msgs.msg.Pose.serialize(val, buffer, bufferOffset);
    });
    // Serialize message field [limb]
    bufferOffset = _serializer.string(obj.limb, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type GoToPointGoal
    let len;
    let data = new GoToPointGoal(null);
    // Deserialize message field [desired_pose]
    // Deserialize array length for message field [desired_pose]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.desired_pose = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.desired_pose[i] = geometry_msgs.msg.Pose.deserialize(buffer, bufferOffset)
    }
    // Deserialize message field [limb]
    data.limb = _deserializer.string(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    let length = 0;
    length += 56 * object.desired_pose.length;
    length += object.limb.length;
    return length + 8;
  }

  static datatype() {
    // Returns string type for a message object
    return 'baxter_msgs_mine/GoToPointGoal';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '844bd63424146f6f878285d0c03bbeb2';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # ====== DO NOT MODIFY! AUTOGENERATED FROM AN ACTION DEFINITION ======
    #goal
    geometry_msgs/Pose[] desired_pose
    string limb
    
    ================================================================================
    MSG: geometry_msgs/Pose
    # A representation of pose in free space, composed of position and orientation. 
    Point position
    Quaternion orientation
    
    ================================================================================
    MSG: geometry_msgs/Point
    # This contains the position of a point in free space
    float64 x
    float64 y
    float64 z
    
    ================================================================================
    MSG: geometry_msgs/Quaternion
    # This represents an orientation in free space in quaternion form.
    
    float64 x
    float64 y
    float64 z
    float64 w
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new GoToPointGoal(null);
    if (msg.desired_pose !== undefined) {
      resolved.desired_pose = new Array(msg.desired_pose.length);
      for (let i = 0; i < resolved.desired_pose.length; ++i) {
        resolved.desired_pose[i] = geometry_msgs.msg.Pose.Resolve(msg.desired_pose[i]);
      }
    }
    else {
      resolved.desired_pose = []
    }

    if (msg.limb !== undefined) {
      resolved.limb = msg.limb;
    }
    else {
      resolved.limb = ''
    }

    return resolved;
    }
};

module.exports = GoToPointGoal;
