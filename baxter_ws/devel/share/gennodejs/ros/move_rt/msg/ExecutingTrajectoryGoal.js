// Auto-generated. Do not edit!

// (in-package move_rt.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class ExecutingTrajectoryGoal {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.trajectory_name = null;
      this.ee_error_th = null;
    }
    else {
      if (initObj.hasOwnProperty('trajectory_name')) {
        this.trajectory_name = initObj.trajectory_name
      }
      else {
        this.trajectory_name = '';
      }
      if (initObj.hasOwnProperty('ee_error_th')) {
        this.ee_error_th = initObj.ee_error_th
      }
      else {
        this.ee_error_th = 0.0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type ExecutingTrajectoryGoal
    // Serialize message field [trajectory_name]
    bufferOffset = _serializer.string(obj.trajectory_name, buffer, bufferOffset);
    // Serialize message field [ee_error_th]
    bufferOffset = _serializer.float64(obj.ee_error_th, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type ExecutingTrajectoryGoal
    let len;
    let data = new ExecutingTrajectoryGoal(null);
    // Deserialize message field [trajectory_name]
    data.trajectory_name = _deserializer.string(buffer, bufferOffset);
    // Deserialize message field [ee_error_th]
    data.ee_error_th = _deserializer.float64(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    let length = 0;
    length += object.trajectory_name.length;
    return length + 12;
  }

  static datatype() {
    // Returns string type for a message object
    return 'move_rt/ExecutingTrajectoryGoal';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'd8f8c747702d98a2b465764ab182b654';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    # ====== DO NOT MODIFY! AUTOGENERATED FROM AN ACTION DEFINITION ======
    # Define the goal
    string trajectory_name
    float64 ee_error_th  # Specify error threshold
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new ExecutingTrajectoryGoal(null);
    if (msg.trajectory_name !== undefined) {
      resolved.trajectory_name = msg.trajectory_name;
    }
    else {
      resolved.trajectory_name = ''
    }

    if (msg.ee_error_th !== undefined) {
      resolved.ee_error_th = msg.ee_error_th;
    }
    else {
      resolved.ee_error_th = 0.0
    }

    return resolved;
    }
};

module.exports = ExecutingTrajectoryGoal;
